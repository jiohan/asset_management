...
3개 테이블 모두 동일한 패턴을 적용한다: 기존 테이블(accounts, categories, transactions)과 동일하게 `FOR ALL USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid())` 단일 정책을 사용한다. investment_assets는 공유 참조 데이터가 아니라 사용자별 소유 데이터이므로 다른 사용자의 종목을 볼 수 없다.
...
`user_id` FK(`investment_assets.user_id`, `investment_trades.user_id`, `investment_events.user_id`)에는 `ON DELETE CASCADE`를 반드시 명시한다 — 기존 테이블(accounts, categories, transactions) 패턴과 동일. 누락 시 PostgreSQL 기본값인 RESTRICT가 적용되어 사용자 계정 삭제 시 연쇄 삭제가 깨진다.
...
`src/features/investments/` 디렉터리 아래에 schemas.ts, queries.ts, actions.ts, holdings.ts를 순서대로 작성한다. balance-calculator.ts 확장(TradeForBalance, EventForBalance 타입, calculateLiquidAssets, calculateInvestmentValue 함수)은 이미 완료된 상태이므로 건드리지 않는다.

**3단계: Vitest 단위 테스트**

holdings.ts에 대한 단위 테스트를 작성하고 통과시킨다. balance-calculator.ts 단위 테스트(투자 계좌 케이스 포함)는 이미 완료된 상태다. UI 작업 전에 핵심 로직이 올바른지 확인한다.
...
`createTrade(formData)`: 매수/매도 기록. account_id는 is_active = true인 투자 계좌만 허용한다. 매도 시 초과 매도 검증 포함. 성공 시 `` `/investments/${accountId}` ``, `/accounts`, `/dashboard` 모두 revalidatePath — 잔액이 3곳에 표시되기 때문이다. `/investments/[accountId]`는 동적 경로이므로 문자열 그대로 쓰면 동작하지 않는다. formData에서 account_id를 읽어 보간해야 한다.
