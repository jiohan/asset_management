...
`calculateTotalAssets`는 수정하지 않는다 — 이 함수가 반환하는 값이 "전체 자산"이 된다. 세 숫자의 관계: `전체 자산 = calculateLiquidAssets + calculateInvestmentValue` 가 성립해야 한다.
...
`deleteTrade(formData)`: 매수/매도 삭제. 확인 모달은 UI 레이어에서 처리.

`deleteEvent(formData)`: 이벤트 삭제.

`deleteAsset(formData)`: 종목 삭제. 연결된 trades 또는 events가 있으면 에러 반환. 없으면 하드 삭제.
