...
| 2 | 핵심 장부 기능 | 제약 보강 3건 (transactions 타입·account_type 트리거·integrity fixes) | 지출 입력→목록 확인→계좌 잔액 반영까지 동작 | [slice2.md](slice2.md) |
...
3. DB 확인: profiles 1행, categories 14행 생성됨 (계좌는 /setup-account에서 수동 등록)
4. 로그아웃 → `/login` 이동
...
**DB 마이그레이션**: 제약 보강 3건 (transactions 타입 제약, account_type 트리거, integrity fixes)

**터치하는 디렉터리**
- `src/features/accounts/` — 계좌 CRUD, 잔액 계산 (`balance-calculator.ts`)
- `src/features/categories/` — 카테고리 조회 (CRUD는 Phase 6)
